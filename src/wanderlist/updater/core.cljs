...
(defn updater [old-store op-type op-data op-id op-time]
  (case op-type
    :add-group
      (update
       old-store
    ...
    :touch-group (assoc-in old-store [:groups op-data :touched-time] op-time)
    :add-task
      (assoc-in
       old-store
    ...
    :hide-sidebar (assoc old-store :show-sidebar? false)
    :show-sidebar (assoc old-store :show-sidebar? true)
    old-store))
